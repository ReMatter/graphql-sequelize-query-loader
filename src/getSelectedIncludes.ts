...
export function getSelectedIncludes<M extends Model>(args: {
  ...
  const includesFromDependantSelectedFields: IncludeOptions[] = selections
    .filter(
      (selection) => dependenciesByFieldNameByModelName[model.name]?.[selection.name.value],
    )
    .flatMap((selection) => {
      ...
      return dependenciesByFieldNameByModelName[model.name]?.[fieldName].map(
        (dependency) => ({
          model: getIncludeModel(model, dependency.dependentAssociation as string, modelsByAssociationByModelName),
  ...
  const includesFromAssociatedModels = selections
    .filter((selection: FieldNode) => {
      ...
      const includedModel = getIncludeModel(model, fieldName, modelsByAssociationByModelName);
      if (!includedModel) {
        console.warn(
    ...
    .map((selection: FieldNode) => {
      ...
      const { attributes, include, where, paranoid, required } = getFindOptionsForModel({
        model: includedModel,
        selection,
  ...
  return [...includesFromDependantSelectedFields, ...includesFromAssociatedModels];
}